...
**초기 스캐폴드 완료.** `wedding.config.ts` 한 파일로 구동되는 정적 청첩장이 끝까지 렌더되고,
RSVP·방명록·라이트박스·공유 섬이 모두 배선됨. 실제 콘텐츠/이미지/백엔드 연결만 남음.

```
[████████████░░░░░░░░] 스캐폴드·구조 100% / 실제 운영 데이터 0%
```
...
| 배포(GitHub Pages) | 🟡 코드·워크플로 준비 완료 (repo 생성·push·Pages 활성화 대기) |
...
### 5. 배포 🟡 (GitHub Pages — 2026-06-17 코드 준비 완료)
**준비 완료(코드):** astro `base`/`site` env 구동 · 에셋/OG URL 중앙 base 보정(`src/lib/config.ts`) ·
`.github/workflows/deploy.yml`(repo 좌표에서 base 자동 도출, `.env` 주입 후 빌드→Pages). 로컬 base 빌드 검증 통과.
- [ ] GitHub repo 생성 + `git push -u origin main`
- [ ] Settings → Pages → Source = **GitHub Actions**
- [ ] Settings → Secrets and variables → Actions → **Variables** 에 3개 등록
      (`PUBLIC_SUPABASE_URL`, `PUBLIC_SUPABASE_ANON_KEY`, `PUBLIC_KAKAO_JS_KEY` — 모두 공개키라 Variables 로 충분)
- [ ] 배포 URL: `https://<유저>.github.io/<repo>/`
> 참고: GitHub 무료 Pages 는 **public repo** 필수 + 대역폭 soft-limit 100GB/월(청첩장 규모엔 충분).
> 소스가 공개되므로 실제 PII(계좌·전화) 입력 시 노출 인지 필요. base path 때문에 **유저사이트가 아닌 프로젝트 repo**.
